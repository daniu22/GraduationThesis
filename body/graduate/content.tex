...
基于以上两个特征，浮动基描述成为了现在对腿足运动通用的表示方法。如\aref{fig:floating_base}所示，我们假设双足机器人和环境之间存在$n_b$个虚拟关节，它们的数值组成了浮动基的坐标$\boldsymbol{q}_b$，
...
该架构在实际硬件平台上得到了验证，可以实现双足机器人的高速稳定行走，和对期望轨迹的稳定跟随。
\section{实验平台介绍}
\begin{figure}[htbp]
    \centering
...
\end{figure}
本文研究基于双足机器人WuKong-IV实验平台实现，实物图见\aref{fig:wukong_pic}，WuKong-IV双
...
为了验证上述轨迹优化算法的有效性，我们设计了双足机器人上台阶实验。实验过程如\aref{fig:track_exp}所示。实验结果表明优化出的质心轨迹相对直接插值得到
